...
apm_query = """query GetSpans {
  actor {
    account(id: 1234567) {
      nrql(query: "SELECT timestamp, duration, request.uri from Transaction WHERE appName = 'FlaskApp - Heroku (APM)' SINCE 60 DAYS AGO LIMIT MAX") {
        results
      }
    }
  }
...
# Convert UNIX epoch time to YYYY-MM-DD HH:mm:ss AM/PM
df.sort_values(by=["timestamp"], inplace=True)
df["timestamp"] = pd.to_datetime(df["timestamp"], unit="ms").dt.strftime(date_format = "%I:%M:%S %p")
#df.head()
